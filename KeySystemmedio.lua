-- ==============================================================================
--         SISTEMA DE PROTEÇÃO EMBUTIDO LOWHIGH STORE - MÉDIO (LEVEL 2)
-- ==============================================================================
...
local TextBox = Instance.new("TextBox", LoginFrame); TextBox.Size = UDim2.new(0, 260, 0, 35); TextBox.Position = UDim2.new(0.5, -130, 0.4, -10); TextBox.BackgroundColor3 = Color3.fromRGB(25, 25, 25); TextBox.Text = ""; TextBox.PlaceholderText = "Cole sua Key aqui... (Médio)"; TextBox.TextColor3 = Color3.fromRGB(220, 220, 220); TextBox.Font = Enum.Font.Gotham; TextBox.TextSize = 12; Instance.new("UICorner", TextBox)
...
Btn.MouseButton1Click:Connect(function()
    ...
    if status then
        -- VERIFICANDO SE A KEY É DO NÍVEL 2 (MÉDIO)
        if KeyAuthApp.user_data.subscriptions[1].level == "2" then
            ...
            print("Acesso MÉDIO Liberado! Duração: " .. expiryStr)
            
            carregarScriptMedium(expiryStr) -- Aciona a função abaixo
        else
            game.Players.LocalPlayer:Kick("❌ LowHigh Store: Acesso NEGADO. Esta key não é do nível MÉDIO (Level 2)!")
        end
    else
...
-- ==============================================================================
    ...
    Title.Position = UDim2.new(0, 10, 0, 12); Title.Size = UDim2.new(0, 200, 0, 20); Title.Text = "Low High Medium"; Title.TextColor3 = Theme.Accent; Title.Font = Enum.Font.GothamBold; Title.TextSize = 16; Title.TextXAlignment = Enum.TextXAlignment.Left; Title.BackgroundTransparency = 1
    ...
    StatusLabel.Position = UDim2.new(1, -165, 0, 32); StatusLabel.Size = UDim2.new(0, 150, 0, 15); StatusLabel.Text = duracaoDaKey; StatusLabel.TextColor3 = Color3.fromRGB(255, 255, 255); StatusLabel.Font = Enum.Font.Gotham; StatusLabel.TextSize = 11; StatusLabel.TextXAlignment = Enum.TextXAlignment.Right; StatusLabel.BackgroundTransparency = 1
    ...
    CreateDropdown(R1, "Type Silent Aim", {"Silent Legit"}, 1, function(val) _G.SilentAimType = val end) 
    ...
    local function GetTargetPart(char, aimType)
        if aimType == "Aimbot Rage" or aimType == "Silent Rage" then return char:FindFirstChild("Head") or char:FindFirstChild("HumanoidRootPart") end
    ...
    RunService:BindToRenderStep("EliteHubMain", Enum.RenderPriority.Camera.Value + 1, function()
        ...
        if _G.AimbotEnabled or _G.SilentAimEnabled then
            ...
            if CachedTarget and CachedTarget.Character then
                if _G.AimbotEnabled then CachedAimbotPos = GetPredictedPosition(CachedTarget, _G.AimbotType) end
                if _G.SilentAimEnabled then CachedSilentPos = GetPredictedPosition(CachedTarget, _G.SilentAimType); CachedSilentPart = GetTargetPart(CachedTarget.Character, _G.SilentAimType) end
            else
                CachedAimbotPos = nil; CachedSilentPos = nil; CachedSilentPart = nil
            end
        else
            CachedTarget = nil; CachedAimbotPos = nil; CachedSilentPos = nil; CachedSilentPart = nil
        end
        ...
        if _G.SilentAimEnabled and CachedTarget and CachedSilentPos and CachedSilentPart then
            if Method == "FireServer" or Method == "InvokeServer" then
                for i, arg in pairs(Args) do
                    if typeof(arg) == "Vector3" then Args[i] = CachedSilentPos elseif typeof(arg) == "CFrame" then Args[i] = CFrame.new(Camera.CFrame.Position, CachedSilentPos) elseif typeof(arg) == "Instance" and arg:IsA("BasePart") then Args[i] = CachedSilentPart end
                end
                return OldNamecall(self, unpack(Args))
            end
            if Method == "Raycast" and self == workspace then Args[2] = (CachedSilentPos - Args[1]).Unit * Args[2].Magnitude; return OldNamecall(self, unpack(Args)) end
        end
    ...
    OldIndex = hookmetamethod(game, "__index", function(self, Index)
        if self == Mouse and _G.SilentAimEnabled and CachedTarget and CachedSilentPos and CachedSilentPart then
            if Index == "Hit" then return CFrame.new(CachedSilentPos) elseif Index == "Target" then return CachedSilentPart end
        end
